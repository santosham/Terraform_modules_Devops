resource "aws_instance" "public-servers" {
    ...
    sed -i '29i <div id="container"><h1>${var.vpc_name}-public-server-${count.index +1}</h1></div' /var/www/html/index.nginx-debian.html
    echo <center><div><h1> ${var.vpc_name}-public-server-${count.index +1}</h1></div>" >></center> /var/www/html/index.nginx-debian.html
    EOF
